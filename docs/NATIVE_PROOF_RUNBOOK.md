...
This runbook turns the repository implementation into reviewer-grade Android/iOS evidence after provider credentials and devices are available. It keeps three claims separate:
...
The web reviewer app is not evidence for levels 2 or 3.
...
- A Firebase project with Android and/or iOS apps registered as `com.hasanalhussein.malreferral`.
...
Branch and Firebase identifiers embedded in mobile config files are not server secrets, but they must belong to projects the applicant or reviewing organization controls. Never commit Branch secrets, service-account keys, signing passwords, or store API keys.
...
```powershell
$packageName = 'com.hasanalhussein.malreferral'
