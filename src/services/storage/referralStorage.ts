...
const BASE_KEYS = {
  ...
  acceptedOutcome: '@mal-referral/accepted-outcome',
...
const LEGACY_KEYS = {
  ...
  frozenAttribution: '@mal-referral/frozen-attribution',
  journey: '@mal-referral/journey',
  processedAttributions: '@mal-referral/processed-attributions',
  milestones: '@mal-referral/analytics-milestones',
  analyticsOutbox: '@mal-referral/analytics-outbox',
...
async function scavengeRetiredEpochs(
  ...
  const retiredKeys = allKeys.filter((key) => {
    if (!key.startsWith('@mal-referral/') || key === ACTIVE_EPOCH_KEY) return false;
