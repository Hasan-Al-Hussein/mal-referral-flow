...
Motion is finite and functional: entry reveals, hover/focus/press feedback, referral-code readiness, reviewer disclosure, signup progress, validation, local-trace changes, link handoff, and completion confirmation. Web controls use a shared 180ms hover language with bounded lift or directional movement; primary CTAs add one finite light sweep. Each route has at most one signature journey surface with restrained ambient depth. Quiet informational surfaces remain positionally stable and use only a faint clipped rim/sheen, without a pointer cursor, tab stop, or press semantics, so motion does not imply a false action. Animations use opacity and transforms, stop on cleanup, never delay analytics or navigation, and become immediately static when reduced motion is enabled. The light-first interface also includes a deliberate dark theme rather than relying on a generic system inversion.
...
The rendered reviewer build was checked at 375 x 812, 768 x 1024, 812 x 375, 1440 x 1000, and a 720px CSS viewport as a 1440-at-200%-zoom reflow proxy, in light and dark themes. The combined Generate → Share → Deferred path was verified at `3/5` on onboarding and `5/5` on completion. On a 375px phone the secure-signup CTA follows the protected referral code before the larger orbit and technical trace, preserving conversion hierarchy. A standalone deferred fixture was separately verified at `1/5 · Click`; signup start and completion advance that invitee-side trace to `3/5 · Verified`. **Run the flow again** was regression-checked to restore a new Invite route at `0/5` without the previous referral code. Reset creates a fresh demo storage epoch, including a newly generated local code. Browser reduced-motion emulation produced no active document animations; native OS preference behavior remains a physical-device check.
