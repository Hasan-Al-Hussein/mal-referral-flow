...
describe('native Branch adapter', () => {
  it('creates the expected Branch payload and always releases the universal object', async () => {
    ...
    const generateShortUrl = jest.fn(() =>
      Promise.resolve({ url: 'https://mal.test-app.link/referral' }),
    );
    ...
    await expect(service.createReferralLink('MAL-ABCD2345')).resolves.toBe(
      'https://mal.test-app.link/referral',
    );

    expect(mockCreateBranchUniversalObject).toHaveBeenCalledWith(
      'referral/MAL-ABCD2345',
      expect.objectContaining({
        contentMetadata: { customMetadata: { referral_code: 'MAL-ABCD2345' } },
      }),
    );
  ...
  it('releases the Branch object when short-link generation fails', async () => {
    ...
    await expect(
      new BranchDeepLinkService().createReferralLink('MAL-ABCD2345'),
  ...
  it('forwards cached/cold or later/warm subscription callbacks without navigation', () => {
    ...
    subscription.onOpenComplete({
      ...
      params: { referral_code: 'MAL-ABCD2345' },
    });
  ...
  it.each([
    { kind: 'direct' as const, code: 'MAL-ABCD2345', firstSession: false },
    { kind: 'deferred' as const, code: 'MAL-ABCD2345', firstSession: true },
    { kind: 'invalid' as const, code: 'BAD-CODE', firstSession: false },
  ])('labels the $kind reviewer fixture without claiming provider delivery', ({
    kind,
    code,
    firstSession,
  }) => {
    ...
    service.simulate(kind, 'MAL-ABCD2345');
