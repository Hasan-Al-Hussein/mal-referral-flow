...
describe('referralStorage', () => {
  ...
  it('replaces a schema-valid wrong-code/platform outbox collision before delivery', async () => {
    ...
    const poisoned: ReferralEventRecord = {
      ...
      properties: {
        ...
        referral_code: 'MAL-ZYXW9876',
        platform: 'ios',
        event_id: 'evt_bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb',
      },
    };
    const outboxKey = await currentEpochKey('@mal-referral/analytics-outbox');
    ...
    expect(JSON.stringify(delivered)).not.toContain('MAL-ZYXW9876');
  ...
  it('canonically rewrites poisoned processed, milestone, outbox, and receipt blobs', async () => {
    const processedKey = await currentEpochKey('@mal-referral/processed-attributions');
    const milestoneKey = await currentEpochKey('@mal-referral/analytics-milestones');
    const outboxKey = await currentEpochKey('@mal-referral/analytics-outbox');
    const receiptKey = await currentEpochKey('@mal-referral/signup-receipts');
  ...
  it('serializes poisoned-set cleanup before a concurrent valid processed write', async () => {
    const key = await currentEpochKey('@mal-referral/processed-attributions');
  ...
  it('migrates valid unscoped baseline and extended state into the active epoch', async () => {
    ...
    await AsyncStorage.multiSet([
      ['@mal-referral/pending-attribution', JSON.stringify(legacyAttribution)],
      ['@mal-referral/frozen-code', CODE],
      ['@mal-referral/processed-attributions', JSON.stringify(['legacy1'])],
      ['@mal-referral/analytics-milestones', JSON.stringify(['invitee:legacy1:clicked'])],
      ['@mal-referral/analytics-outbox', JSON.stringify([legacyEvent])],
      ['@mal-referral/signup-receipts', JSON.stringify({ 'signup:legacy1': legacyReceipt })],
      ['@mal-referral/generated-code/member-legacy', CODE],
    ]);
    reloadReferralStorageRuntime();
    ...
    for (const key of [
      '@mal-referral/pending-attribution',
      '@mal-referral/frozen-code',
      '@mal-referral/processed-attributions',
      '@mal-referral/analytics-milestones',
      '@mal-referral/analytics-outbox',
      '@mal-referral/signup-receipts',
      '@mal-referral/generated-code/member-legacy',
    ]) {
      await expect(AsyncStorage.getItem(key)).resolves.toBeNull();
    }
  });
  ...
  it('canonicalizes an already-split journey to the frozen attribution', async () => {
    ...
    const pending = attribution('MAL-ZYXW9876', 'pending1');
    const key = await currentJourneyKey();
  ...
  it('scavenges a retired epoch namespace during process-style reload', async () => {
    const retiredKey = `@mal-referral/journey:${'f'.repeat(32)}`;
  ...
  it('generation-fences a late startup scavenger from the winning epoch', async () => {
    ...
    const fresh = attribution('MAL-ZYXW9876', 'freshc1');
    await referralStorage.savePendingAttribution(fresh);
  ...
  it('treats hung post-commit namespace cleanup as non-critical', async () => {
    ...
    const fresh = attribution('MAL-ZYXW9876', 'fresh01');
    await referralStorage.savePendingAttribution(fresh);
    await expect(referralStorage.getPendingAttribution()).resolves.toEqual(fresh);
  });
