...
class TrackerStorage implements ReferralStorage {
  ...
  async saveSignupReceipt(
    ...
    _receipt: ReferralAcceptanceReceipt,
  ): Promise<void> {}
...
function createTracker(storage = new TrackerStorage(), client = new RecordingClient()) {
  return {
    storage,
    client,
    tracker: new AnalyticsTracker(client, storage, 'ios', () => NOW),
  };
}
